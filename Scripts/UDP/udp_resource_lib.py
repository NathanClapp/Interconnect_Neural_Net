...
def nested_dict_template(module_name_string, function, function_argc):
    return {module_name_string : {function : function_argc} }

print(nested_dict_template(operator_modules[0], 'testfuncstr', 5))
print(dir(operator_modules[0]))

#get every function and the number of inputs for each
#return a structured json array of module_name(function_name(input_count))
#ignore builtin methods by default
def py_functions_from_modules(modules_list, json_dir, ignore_substring='__'):
    ...
    for module_name in modules_list:
        for function in dir(module_name):
            ...
            if not (function.startswith(ignore_substring) or function.endswith(ignore_substring)):
                function_argc = len(signature(function).parameters)
    #workspot
    return 0;
